...
<head>
    ...
    <title>รายงานรับเข้าพัสดุ</title>
...
<body>
    <div class="container mt-5">
        <div class="header-card text-center p-4 mb-4">
            <h1 class="mb-0"><i class="fas fa-box-open"></i> รายงานรับเข้าพัสดุ</h1>
        </div>
        ...
        <div class="card filter-card mb-4">
            <div class="card-body">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-lg-4 col-md-6">
                        <label for="ComDropdown" class="form-label"><i class="fas fa-building"></i>บริษัทรับเข้า:</label>
                        <select id="ComDropdown" class="form-select" onchange="filterTable()">
                            <option value="">-- ทั้งหมด --</option>
                            <?php
                            $sqlCom = "SELECT DISTINCT h_sec FROM history_db WHERE h_status = 'รับเข้า'";
                    ...
                    <div class="col-lg-4 col-md-6">
                        ...
                        <select id="typeDropdown" class="form-select" onchange="filterTable()">
                            <option value="">-- ทั้งหมด --</option>
                            <?php
                            $sqlType = "SELECT DISTINCT h_type FROM history_db WHERE h_status = 'รับเข้า'";
                    ...
                    <div class="col-lg-4 col-md-6">
                        ...
                        <select id="masterDropdown" class="form-select" onchange="filterTable()">
                            <option value="">-- ทั้งหมด --</option>
                            <?php
                            $sqlMaster = "SELECT DISTINCT h_master FROM history_db WHERE h_status = 'รับเข้า'";
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover align-middle">
                ...
                <?php
                    $sql = "SELECT * FROM history_db WHERE h_status = 'รับเข้า' ORDER BY h_id DESC";
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo "<tr>
                            ...
                            <td class='text-center'><span class='badge bg-success'>" . htmlspecialchars($row['h_status']) . "</span></td>
                            <td>" . htmlspecialchars($row['h_bill']) . "</td>
                            <td>" . htmlspecialchars($row['h_master']) . "</td>
                            <td class='text-center'>
                                <a href='delete_report_r.php?h_id=" . htmlspecialchars($row['h_id']) . "' class='btn btn-outline-danger btn-sm'
    ...
    <script>
        function filterTable() {
            ...
            for (const row of rows) {
                ...
                const secCell = row.cells[5].textContent.toLowerCase(); // คอลัมน์ที่ 6 คือ บริษัท
                const dateCellRaw = row.cells[6].textContent; // คอลัมน์ที่ 7 คือ วันที่
                const masterCell = row.cells[9].textContent.toLowerCase(); // คอลัมน์ที่ 10 คือ ผู้บันทึก
